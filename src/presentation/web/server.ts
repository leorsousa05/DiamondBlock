...
export async function createWebServer(container: Container, options: WebServerOptions) {
  ...
  if (existsSync(options.staticDir)) {
    ...
    app.setNotFoundHandler(async (req, reply) => {
      ...
      return reply.code(200).type('text/html').send(
        `<!DOCTYPE html><html><body><h1>DiamondBlock API</h1>
...
export async function startWebServer(container: Container, options: WebServerOptions): Promise<void> {
