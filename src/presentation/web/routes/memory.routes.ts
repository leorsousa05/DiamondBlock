...
export const memoryRoutes: FastifyPluginAsync<{ container: Container }> = async (app, opts) => {
  ...
  app.get('/api/memories', async (req, reply) => {
    ...
    if (q) {
      ...
      const searchResults = fullMemories.filter(Boolean) as any[];
      if (type) {
  ...
  app.post('/api/memories/purge', async (req, reply) => {
    ...
    const toDelete = all.filter((m) => {
      if (projectId && !m.scope.includes(projectId)) return false;
