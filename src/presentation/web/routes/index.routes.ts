...
export const indexRoutes: FastifyPluginAsync<{ container: Container }> = async (app, opts) => {
  ...
  app.get('/api/index/status', async (req, reply) => {
    ...
    const chunkCount = Object.values(manifest.files).reduce(
    ...
    if (await embeddingProvider.isAvailable()) {
      try {
        ...
        const fullChunks = await Promise.all(
          results.map(async (r) => {
            if (!codebaseChunkRepository) return null;
