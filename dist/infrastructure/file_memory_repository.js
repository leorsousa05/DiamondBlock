...
export class FileMemoryRepository {
    ...
    async searchWithScore(options) {
        ...
        if (options.query) {
            ...
            memories = memories.filter((m) => m.title.toLowerCase().includes(lower) ||
    ...
    async findExistingPath(id) {
        const files = await walkDirectory(this.memoryDir);
        return files.find((file) => file.endsWith('/' + id + '.md') || file.endsWith(id + '.md')) ?? null;
    }
}
//# sourceMappingURL=file_memory_repository.js.map